...
var GatewayStaticAddresses = suite.ConformanceTest{
	...
	Test: func(t *testing.T, s *suite.ConformanceTestSuite) {
		...
		require.Len(t, currentGW.Spec.Addresses, 2, "expected two addresses on the Gateway, one usable and one unusable. somehow got %d", len(currentGW.Spec.Addresses))
		unusableAddress := currentGW.Spec.Addresses[0]
		usableAddress := currentGW.Spec.Addresses[1]

		t.Logf("verifying that the Gateway %s/%s is accepted, but is not programmed due to an address that can't be used", gwNN.Namespace, gwNN.Name)
