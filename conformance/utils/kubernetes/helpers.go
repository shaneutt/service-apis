...
// GatewayExcludedFromSetupAnnotation is an annotation that can be placed on a
...
const GatewayExcludedFromSetupAnnotation = "sigs.k8s.io/gateway-api/skip-in-conformance-test-setup"
	...
	waitErr := wait.PollUntilContextTimeout(context.Background(), 1*time.Second, timeoutConfig.NamespacesMustBeReady, true, func(ctx context.Context) (bool, error) {
		for _, ns := range namespaces {
			...
			for _, gw := range gwList.Items {
				...
				if val, ok := gw.Annotations[GatewayExcludedFromSetupAnnotation]; ok && val == "true" {
